...
def load_data_one():
    ...
    test_feature_range, test_label_range = data_divide(dframe, dframe_test)
...
def data_divide(dframe, dframe_test):
    # 训练1
...
def pre_process_data(dframe, dframe_test):
    dframe['category'] = dframe['item_category_list'].apply(
        lambda x: x.split(";")[2] if len(x.split(";")) == 3 else x.split(";")[1])
    dframe_test['category'] = dframe_test['item_category_list'].apply(
        lambda x: x.split(";")[2] if len(x.split(";")) == 3 else x.split(";")[1])
    ...
    dframe['item_category_with_predict_category_intersection_len'] = dframe['item_category_preditct_category'].apply(
        get_category_intesection)
    dframe_test['item_category_preditct_category'] = dframe_test['item_category_list'] + '-' + dframe_test[
        'category_list']
    dframe_test['item_category_with_predict_category_intersection_len'] = dframe_test[
        'item_category_preditct_category'].apply(get_category_intesection)
    ...
    dframe['predict_property_jiaoji_item_property'] = dframe['item_property_preditct_property'].apply(
        get_property_intesection)
    dframe_test['preditct_property_list'] = dframe_test['predict_category_property'].apply(get_property_list)
    dframe_test['item_property_preditct_property'] = dframe_test['item_property_list'] + '-' + dframe_test[
        'preditct_property_list']
    dframe_test['predict_property_jiaoji_item_property'] = dframe_test['item_property_preditct_property'].apply(
        get_property_intesection)
...
def main():
    ...
    ftrain = pd.concat([train1, train2, train3, train4, train5, train6, train7], axis=0)
