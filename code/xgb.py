...
def xgb_model_val():
    ...
    df.to_csv("feature_imporant.csv",sep=" ",index=None)
...
def xgb_model_test():
    ...
    train_x = train_set.drop(['instance_id','context_id','item_city_id','item_id','user_id','item_brand_id','shop_id','user_gender_id','user_occupation_id',
                              'context_timestamp','context_page_id','context_timestamp_and_dates','dates','day','hour','item_category_list',
                              'item_property_list','predict_category_property','is_trade','property','property_max_count','user_and_category_rank','user_and_category_rank_desc',
                              'aa_user_item_shop_rank_label', 'aa_user_item_shop_rank_desc_label','user_category_count_label'],axis=1)
    train_y = train_set['is_trade']
    ...
    test_x = test_set.drop(['instance_id','context_id','item_city_id','item_id','user_id','item_brand_id','shop_id','user_gender_id','user_occupation_id',
                            'context_timestamp','context_page_id','context_timestamp_and_dates','dates','day','hour','item_category_list',
                            'item_property_list','predict_category_property','property','property_max_count','user_and_category_rank','user_and_category_rank_desc',
                            'aa_user_item_shop_rank_label','aa_user_item_shop_rank_desc_label','user_category_count_label'],axis=1)

    xgb_train = xgb.DMatrix(train_x.values, label=train_y)
...
def gene_result(pred_value,test_range):
    ...
    res.to_csv("submit/result_182.submit", index=None,sep=' ',line_terminator='\r')
